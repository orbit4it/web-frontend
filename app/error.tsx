    ...
    <main className="w-full h-screen py-10 flex flex-col items-center  bg-primary  bg-blend-screen bg-cover bg-center text-white overflow-x-hidden relative">
      ...
      >
        <h1 className="text-5xl font-semibold z-10">
          Error
          {error.stack?.startsWith('UnderConstructionError') && <span>🚧</span>}
        </h1>
        <h2 className="text-2xl font-bold my-5 z-10">{error.message}</h2>
        {error.stack?.startsWith('UnderConstructionError') ? (
          <button
          ...
          <button
            className="bg-darkBlue rounded-lg px-3 py-2 mt-3 text-white absolute bottom-5"
            onClick={reset}
          >
            Reload 🔃
          </button>
